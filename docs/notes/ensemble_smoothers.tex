...
Many procedures have been proposed to alleviate it, such as, e.g., setting all correlations that are below a $3 / \sqrt{r}$ to zero \citep{vossepoel2025adaptive}.
...
\end{align*}
We have seen that there are different way to express the update equations: the precision form of \cref{eqn:posterior_cov_precision}, the Schur form of \cref{remark:posterior_covariance} and the Kalman form in \cref{eqn:posterior_mean_kalman}.
...
\begin{equation}
    ...
    \frac{XY^T}{r-1}
    \left(
    \Sigma_\epsilon + \frac{YY^T}{r-1}
    \right)^{-1} \left( \bm{d} \bm{1}^T - (Y + E) \right).
\end{equation}
This equation should never be implemented naïvely.
For instance, the inverse should not be explicitly formed and neither should the huge cross-covariance matrix $XY^T$ (order of matrix multiplication matters).
...
The hope is that if we are able to compute high-quality estimates of $H$ and $\Lambda_x^{-1}$, the resulting update will also have high quality.
...
\begin{itemize}
    \item If $X_\text{prior}$ is known to be multivariate Gaussian, e.g. $\bm{x}_\text{prior} \sim \mathcal{N}\left( \bm{x} \mid \bm{\mu}_x, \Sigma_x \right)$ with a known $\Sigma_x$, then $\Lambda_x = \Sigma_x^{-1}$ and no estimation is needed. % Often $\Sigma_x$ is diagonal.
    \item Sometimes $X_\text{prior}$ is given without any information about its distribution, but the adjacency matrix $G$ is given ($G_{ij} = 1$ iff $(\Lambda_x)_{ij} \neq 0$) (e.g. a 5-point stencil).
    Then we have to solve the problem ``estimate $\Lambda_x$, given $X_\text{prior}$ and $G$''.
    ...
    \item If $X_\text{prior}$ is given but no information about the graph structure $G$ is given, then the graph structure should be estimated.
...
Case one:
If $X_\text{prior}$ is multivariate Gaussian, e.g. $\bm{x}_\text{prior} \sim \mathcal{N}\left( \bm{x} \mid \bm{\mu}_x, \Sigma_x \right)$ with a known $\Sigma_x$, then $\Lambda_x = \Sigma_x^{-1}$ and no estimation is needed.
If $X_\text{prior}$ is given without any information about distribution, but the adjacency matrix $G$ is given ($G_{ij} = 1$ iff $(\Lambda_x)_{ij} \neq 0$)
