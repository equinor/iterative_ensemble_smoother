...
To implement ES, or any ES-like algorithm, we must compute the ES update \cref{eqn:ensemble_smoother_update} given in \cref{statement:es}.
...
\end{equation}
We should not, under any circumstance, implement this equation as-is.
With some knowledge of practical linear algebra, massive improvements in both numerical accuracy and speed are possible over the naïve implementation.
For instance:
\begin{itemize}
    \item Do not explicitly form the inverse of $(\Sigma_\epsilon + YY^T / (r-1))$.
    Instead formulate it as the solution to a linear system.
    \item The order matrix multiplications matters a lot.
    For instance, avoid forming the huge $XY^T$ cross-covariance matrix of shape $m \times n$ (parameters $\times$ responses).
    \item The best way I have found to compute the update is to first take the Cholesky factor of $\Sigma_\epsilon$, which is needed to draw the columns of $E$ (recall $E_{\cdot, j} \sim \mathcal{N}(\bm{0}, \Sigma_\epsilon)$) anyway, and call it $L L^T = \Sigma_\epsilon$.
    Then use that to ``whiten'' $(\Sigma_\epsilon + YY^T / (r-1))$ so and then do so-called subspace inversion.
    This approach is extensively documented \href{https://github.com/equinor/iterative_ensemble_smoother/blob/140522402eb13ff3df5ec51027c8faca974252ae/src/iterative_ensemble_smoother/esmda_inversion.py}{iterative\_ensemble\_smoother}.
\end{itemize}
