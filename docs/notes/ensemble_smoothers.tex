...
Ensemble smoother algorithms are used to approximate a posterior distribution.
Given a prior over parameters, a forward model (e.g. a petroleum reservoir model) and real-world observations: what is the posterior over the parameters?
Ensemble-based algorithms describe the posterior with a set of samples, and the most common use-case is when the forward model is black-box and expensive to evaluate.
...
The equation for the Gaussian posterior is the starting point for all ensemble methods.
We derive the Ensemble Smoother (ES)---an algorithm that retains some properties of the Gauss-linear case.
...
In high dimensions with few samples, empirical correlations are mostly noise and very little signal.
This phenomenon is called \emph{spurious correlations}, and all the methods above suffer from it.
Many procedures have been proposed to alleviate it, such as setting all correlations that are below $3 / \sqrt{r}$ to zero \citep{vossepoel2025adaptive}.
...
With a Gaussian prior, a Gaussian noise model and a linear forward model, we're in the happy Gauss-linear case where an analytical expression for the posterior can be derived.
In this section we establish notation and illustrate several expressions for the posterior.
...
The parameters $\bm{x}$ are uncertain static properties of the subsurface, e.g., porosity.
The forward model $h$ is a reservoir flow simulator.
It solves the fluid flow equations over the grid and reports predicted production data $\bm{y}$, for instance oil production at a well.
A single evaluation of $h$ may take hours of compute, and we only have access to its inputs and outputs.
The match is never exact, so the real-world observation $\bm{d}$ departs from the simulator response by a noise term $\bm{\epsilon}_d$.
The inverse problem, called \emph{history matching}, is: given the observed production $\bm{d}$, how can we update our knowledge of the parameters $\bm{x}$?
...
\begin{remark}[Response covariance]
    ...
    In both equations below we'll use the fact that $\bm{\epsilon}_d$ is independent of $\bm{x}$.
...
\end{align*}
Note also that $\Sigma_{xy} =
\operatorname{Cov}\left[ \bm{x}, \bm{y} \right] =
\operatorname{Cov}\left[ \bm{x}, H\bm{x} \right] =
\operatorname{Cov}\left[\bm{x}, \bm{x}\right] H^T =
 \Sigma_{xd}$.
\end{remark}

Suppose the prior over the parameters $\bm{x}$ and the observation $\bm{d}$, conditioned on the parameters, are given by multivariate Gaussians:
\begin{equation}
    \label{eqn:model_def_gaussian}
    \begin{aligned}
...
\end{align}
The equations above are well known, and can for instance be found in \citet[Section~2.3.3]{BISH11}.\footnote{We can derive the posterior mean $\bm{\mu}_{x \mid d}$ by maximizing the log-posterior $\log ( p(\bm{x} \mid \bm{d}))$, which is equal to  $\log ( p(\bm{d} \mid \bm{x}) \, p(\bm{x}))$ (up to a constant) by Bayes' theorem.
...
In the equations above, the posterior mean $\bm{\mu}_{x \mid d}$ is expressed in terms of precision matrices $\Sigma_x^{-1}$ and $\Sigma_\epsilon^{-1}$ rather than covariance matrices $\Sigma_x$ and $\Sigma_\epsilon$.
...
The Kalman gain form is an alternative way to express the update equations above, and we'll spend the rest of this section deriving it.
It's more commonly used in the Kalman filter literature, and the update is given by:
\begin{align}
    \label{eqn:posterior_mean_kalman}
...
\end{align}
In the equation above, $H \bm{\mu}_{x}$ is the predicted mean, $(\bm{d} - H \bm{\mu}_{x})$ is the \emph{innovation} term and $K$ is the \emph{Kalman gain}.
In words: the posterior equals the prior, plus a linear combination of the innovation (residuals).

We will now derive the Kalman update in \cref{eqn:posterior_mean_kalman}.
...
To see this, we start by assuming that both expression are equal:
\begin{equation}
...
The second step is to realize that, using \cref{remark:posterior_covariance}, we can write
\begin{align*}
...
With these two results in hand, we start with \cref{eqn:posterior_mean_precision} and show that it can indeed be transformed into \cref{eqn:posterior_mean_kalman}.
The update equation for the mean becomes
\begin{align*}
    \bm{\mu}_{x \mid d} &=
    \Sigma_{x \mid d} \left(
    H^T \Sigma_\epsilon^{-1} \bm{d} + \Sigma_x^{-1} \bm{\mu}_x
...
\end{align*}
We have seen that there are different ways to express the update equations: the precision form of \cref{eqn:posterior_cov_precision}, the Schur form of \cref{remark:posterior_covariance} and the Kalman form in \cref{eqn:posterior_mean_kalman}.
Which one should we use?
It depends on what we wish to show or prove, or how we want to structure our computations.
For instance, the Ensemble Smoother---which we discuss next---uses a Kalman style update with covariances.
...
Then we propagate them through the forward model $h$ and obtain $\bm{y}_j = h(\bm{x}_j)$ for each realization $j=1, \ldots, r$.
Finally, we perform \emph{data assimilation}; a Kalman-style update that transforms the prior ensemble to the posterior ensemble by conditioning on the observation.
...
Since the forward model is expensive to compute, the number of realizations $r$ is much smaller than the number of parameters $m$ and responses $n$.
This leads to poor estimates of covariances due to spurious correlations.
...
The vector $\bm{1}$ is a vector of $r$ ones, so $\bm{d} \bm{1}^T$ is a matrix with the observation $\bm{d}$ repeated in each of the $r$ columns.
The matrices $\hat{\Sigma}_{xy}$ and $\hat{\Sigma}_y$ are estimates of the cross-covariance and the covariance.
The most common estimate for, e.g.,  $\hat{\Sigma}_{xy}$ is the empirical cross covariance matrix $\bar{X}\bar{Y}^T / (r-1) \in \R^{m \times n}$,
...
We'll spend the rest of this section showing that \cref{eqn:ensemble_smoother_update} is the correct update equation in the following sense: when we use it on a linear model and the number of realizations $r$ goes to infinity, the posterior mean and posterior covariance matches the analytical posterior.
...
The update equation for each realization $j$, based on \cref{eqn:ensemble_smoother_update}, is given by
\begin{equation*}
...
\end{align*}
where $\bm{d}$ is no longer a random variable, but an actual real-world observation.
The residual term $(\bm{d} - (H \bm{x}_{\text{prior}} + \bm{\epsilon}_d))$ is the actual value minus the predicted value.

First we show that the posterior mean equals \cref{eqn:posterior_mean_kalman}:
\begin{align*}
...
More specifically, we show it to be equal to the Schur complement form derived in \cref{remark:posterior_covariance}.
Below we use the fact that $K\bm{d}$ is constant and that $\operatorname{Cov} \left[ \bm{x}_{\text{prior}},  \bm{\epsilon}_d \right] = 0$ when we distribute the covariance over the terms.
\begin{align*}
...
\begin{equation*}
    ...
    = \Sigma_{x \mid d},
\end{equation*}
where the last equality follows from \cref{remark:posterior_covariance}.

In summary, we have shown that if we use \cref{eqn:ensemble_smoother_update} to update the ensemble, then the resulting posterior mean and covariance over the realizations matches the analytical posterior in the Gauss-linear case.
...
To implement ES we must compute \cref{eqn:ensemble_smoother_update} given in \cref{statement:es}.
Substituting empirical population estimates for $\hat{\Sigma}_{xy}$ and $\hat{\Sigma}_y$, the update equation becomes
\begin{equation}
    \nonumber
    X_{\text{posterior}}
    = X_{\text{prior}} +
    \frac{\bar{X}\bar{Y}^T}{r-1}
    \left(
    \Sigma_\epsilon + \frac{\bar{Y}\bar{Y}^T}{r-1}
    \right)^{-1} \left( \bm{d} \bm{1}^T - (Y + E) \right).
\end{equation}
This equation should never be implemented naïvely.
For instance, the inverse should not be explicitly formed and neither should the huge cross-covariance matrix $\bar{X}\bar{Y}^T$ (order of matrix multiplication matters).

\citet[Appendix~A]{emerick2012esmda} write about computation, and explain two tricks: (1) scaling the system of equations and (2) subspace inversion.
\citet[Appendix~B]{emerick2016analysis} also discusses computation.
But their sentence ``it is computationally more efficient to rescale based only on the diagonal elements'' is not as clear as it could be: using only the diagonal elements means we don't solve the original equation.

The best way I know to solve this system is to scale with the Cholesky factor of $\Sigma_{\epsilon}$, then do a subspace inversion with a single SVD.
This is different from both papers above, and is documented \href{https://github.com/equinor/iterative_ensemble_smoother/blob/140522402eb13ff3df5ec51027c8faca974252ae/src/iterative_ensemble_smoother/esmda_inversion.py}{iterative\_ensemble\_smoother}.

%\subsection{Regularization}
%
%One path to improving ES is to improve the estimates $\hat{\Sigma}_{xy}$ and $\hat{\Sigma}_y$, and therefore the Kalmain gain esimate $\hat{K}$.
%
%There is rich literature on high dimensional covariance estimation, for instance \citet{ledoit2004honey} and \citet{bickel2008covariance}.
%
...
When $m$ and $n$ are larger than $r$, empirical covariance is a poor estimator that picks up spurious correlations---the update ends up being dominated by noise.
...
The \emph{information} terminology means that we parametrize in terms of precision rather than covariance.
We're discussing smoothers, so the \emph{filter} does not exactly match this document---but we'll stick with EnIF rather than inventing a new abbreviation.
...
If we model $\bm{r} \sim \mathcal{N}(\bm{0}, \Sigma_{r})$, then a large $\Sigma_{r}$ means the linear model is poor.
...
We will now sketch why \cref{eqn:ensemble_smoother_update_inf} is the correct update equation, in the sense that posterior mean and covariance matches under the Gauss-linear model.
...
\end{align*}
This is analogous to \cref{eqn:model_def_gaussian}, but with $\Sigma_{\epsilon}$ replaced by $\Sigma_{\epsilon} + \Sigma_{r}$.
...
and $\Lambda_x = \Sigma_{x}^{-1}$ by definition.
...
We have no theoretical guarantees that EnIF is better than ES, in the sense that the update equation is equivalent but written in a different form.
The hope is that if we are able to compute high-quality estimates of $H$ and $\Lambda_x$, the resulting update will also have high quality.

\subsection{Computation}
Here are some notes on computation of the EnIF update.

\textbf{Subproblem 1 (Estimate $\Lambda_x$)}
\begin{itemize}
    \item If the realizations in $X_\text{prior}$ are known to be multivariate Gaussian, e.g. $\bm{x}_\text{prior} \sim \mathcal{N}\left( \bm{x} \mid \bm{\mu}_x, \Sigma_x \right)$ with a known $\Sigma_x$, then $\Lambda_x = \Sigma_x^{-1}$ and no estimation is needed. % Often $\Sigma_x$ is diagonal.
    \item If $X_\text{prior}$ is given without information about its distribution, but the adjacency matrix $G$ is given ($G_{ij} = 1$ iff $(\Lambda_x)_{ij} \neq 0$), then we have to solve the problem ``estimate $\Lambda_x$, given $X_\text{prior}$ and $G$.''
    This can for instance be done with the algorithm given in \citet[Chapter~17.3.1]{hastie2009elements} or the triangular transport approach given in \citet*{lunde2025ensemble}.
    \item If $X_\text{prior}$ is given but no information about the graph structure $G$ is given, then the graph structure can be estimated.
    A good starting point for information about this problem is \citet[Chapter~17.3.2]{hastie2009elements}.
\end{itemize}

\textbf{Subproblem 2 (Estimate $H$)}

The problem is to find $H \in \mathbb{R}^{n \times m}$ such that $Y \approx H X$.
This can be formulated as $n$ regressions, one for each response: $X^T H^T \approx Y^T$ in matrix notation or $X^T \bm{h}_{i, \cdot} \approx \bm{y}_{i, \cdot}$ in vector notation.
For each response $i$, we solve the regression problem to obtain $\bm{h}_{i, \cdot}$, the $i$'th row in $H$.

This regression should be
\begin{itemize}
    \item Sparse, since then the posterior precision $\Lambda_x + H^T \tilde{\Lambda} H$ will retain most of the sparsity of the prior $\Lambda_x$.
    Sparsity also means a more interpretable model.
    \item Fast, since both $m$ and $n$ can be huge.
    \item High dimensional, since $r$ is small compared to $m$ and $n$.
\end{itemize}
Some kind of Lasso or boosted linear regression is a good choice.
See \citet[Appendix~E]{lunde2025ensemble} for more details.
The information criterion in \citet{boosting2006buhlman} could perhaps be used.

\textbf{Main computation}
...
\begin{enumerate}
    \item Compute the residual matrix $R = Y - HX_{\text{prior}}$ and estimate
    $\Sigma_r$ from it. For instance by choosing $\Sigma_r$ to be diagonal, i.e., taking the variance over each row (response-residual) in $R$.

    \item Compute $\tilde{\Lambda} = \left( \Sigma_{\epsilon} + \Sigma_{r} \right)^{-1}$.
    If either $\Sigma_{\epsilon}$ or $\Sigma_{r}$ is dense, then do not explicitly compute the inverse like this, but instead use e.g. Cholesky factorization to solve systems of equations.
    If they are both diagonal, then exploit that fact in computation instead of actually forming the diagonal matrix.

    \item Solve
    \begin{equation*}
        ( \Lambda_x + H^T \tilde{\Lambda} H ) Z = H^T \tilde{\Lambda}
        (\bm{d} \bm{1}^T - (Y + E))
    \end{equation*}
    or, equivalently, the column version for each column $j$
    \begin{equation*}
        ( \Lambda_x + H^T \tilde{\Lambda} H ) \bm{z}_j = H^T \tilde{\Lambda}
        (\bm{d} - (\bm{y}_j + \bm{\epsilon}_d))
    \end{equation*}
