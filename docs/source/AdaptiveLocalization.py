...
A = SplineTransformer(n_knots=num_parameters - 2).fit_transform(
    np.linspace(0, 1, num=num_observations)[:, np.newaxis]
)
assert A.shape == (num_observations, num_parameters)
...
# SETUP ESMDA FOR LOCALIZATION AND SOLVE PROBLEM
# =============================================================================
alpha = normalize_alpha(np.ones(ESMDA_ITERATIONS))
...
print("Correlation Ridge", pearsonr(x_true, model.coef_).statistic)
print("Correlation ESMDA", pearsonr(x_true, X_i_means).statistic)
print("Correlation Adap. ESMDA", pearsonr(x_true, X_i_means_adaptive).statistic)
